...
const MarketPage = () => {
    ...
    const columns: TableColumnDefinition<MarketItem>[] = [
        ...
        createTableColumn<MarketItem>({ columnId: 'exchange', renderHeaderCell: () => t('col_exchange'), renderCell: (item) => <Badge appearance="tint" size="small">{item.exchange}</Badge> }),
        ...
        createTableColumn<MarketItem>({ columnId: 'change', compare: (a, b) => a.change_absolute - b.change_absolute, renderHeaderCell: () => t('col_change'), renderCell: (item) => { const val = Number(item.change_absolute); return <span className={val >= 0 ? styles.pos : styles.neg} style={{ fontWeight: 600 }}>{val > 0 ? '+' : ''}{val.toFixed(4)}</span>; } }),
