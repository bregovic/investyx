...
export default class CsvRecognizer extends BaseRecognizer {
  async identify(content, filename = '') {
    ...
    // Some CSVs might be parsed slightly differently (e.g. empty strings vs null), so we rely on key columns order
    // But user suggested checking the full line. Since `content` is already parsed to 2D array, we reconstruct the first line.
    if (content[0]) {
