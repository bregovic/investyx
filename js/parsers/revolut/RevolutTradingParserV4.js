...
export default class RevolutTradingParser extends BaseParser {
    ...
    parsePdf(text) {
        ...
        for (const chunk of chunks) {
            ...
            // Trade - support USD and EUR
                ...
                const isCrypto = ['BTC', 'ETH', 'XRP', 'LTC', 'BCH', 'ADA', 'DOT', 'SOL', 'MATIC', 'AVAX', 'DOGE', 'SHIB', 'LINK', 'UNI'].includes(match[1].toUpperCase());
                ...
                tx = {
                    ...
                    currency: currency,
                    platform: 'Revolut',
                    product_type: isCrypto ? 'Crypto' : 'Stock',
                    trans_type: side === 'BUY' ? 'Buy' : 'Sell',
                    fees: fees + extra,
                    notes: `Trade - ${match[2]} ${side}`
                };

                // Auto-generate crypto fee transaction (Sell to reduce balance)
                if (isCrypto && fees > 0 && price > 0) {
                    const feeInCrypto = fees / price;
                    transactions.push(tx); // Push main tx first
                    ...
                    transactions.push({
                        ...
                        trans_type: 'Sell',
                        fees: 0,
                        notes: `Auto-fee: ${fees} ${match[9] || ''} converted`
                    });

                    tx = null; // Prevent double push at the end of loop
                }
            }
            // Corporate actions
    ...
    parseRow(row, columnMap) {
        ...
        // Heuristika pro detekci Krypto (Revolut v PDF často uvádí vše jako Trade)
        const knownCrypto = ['BTC', 'ETH', 'SOL', 'XRP', 'ADA', 'DOT', 'DOGE', 'SHIB', 'LTC', 'MATIC', 'AVAX', 'UNI', 'LINK', 'ATOM', 'ALGO', 'XLM'];
        if (productType === 'Stock' && knownCrypto.includes(ticker)) {
            productType = 'Crypto';
        }
