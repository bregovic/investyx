...
function upsert($pdo,$date,$code,$amount,$rate,$src,&$ins,&$upd){
    $s=$pdo->prepare("SELECT rate_id FROM broker_exrates WHERE date=? AND currency=? LIMIT 1");
    $s->execute([$date,$code]); $id=$s->fetchColumn();
    if($id){ $u=$pdo->prepare("UPDATE broker_exrates SET rate=?,amount=?,source=?,updated_at=CURRENT_TIMESTAMP WHERE rate_id=?"); $u->execute([$rate,$amount,$src,$id]); $upd++; }
    else { $i=$pdo->prepare("INSERT INTO broker_exrates (date,currency,rate,amount,source) VALUES (?,?,?,?,?)"); $i->execute([$date,$code,$rate,$amount,$src]); $ins++; }
}
